...
func (c *Client) streamOpts(s *StreamConfig) []stream.Option {
	...
	return []stream.Option{
		...
		stream.WithDisconnectCallback(func() { c.l.Warn("stream was disconnected", "url", x) }),
		stream.WithConnectCallback(func() { c.l.Info("stream connected", "url", x) }),
